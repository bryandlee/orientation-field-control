function [A,B,Theta] = Cal_ABTheta(T,W,order_phi)
...
x = T(1:dim,dim+1,:);
R = T(1:dim,1:dim,:);
ss = size(Phi(reshape(x(:,1,1),[dim,1]),order_phi));
M = ss(1);
...
for i=1:N
    for j=1:dim
        A(j,:,:) = A(j,:,:) + reshape(x(j,1,i) * Phi(reshape(x(:,1,i),[dim,1]),order_phi) * r(i,:),[1,M,dim*(dim-1)/2]);
    end
    for j=1:dim
        for k=1:dim
            B(j,k,:,:) = B(j,k,:,:) + reshape( x(j,1,i) * x(k,1,i)* Phi(reshape(x(:,1,i),[dim,1]),order_phi) * Phi(reshape(x(:,1,i),[dim,1]),order_phi)', [1,1,M,M] ) ;
        end
    end
end
